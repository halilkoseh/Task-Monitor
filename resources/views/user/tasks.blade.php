                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 task-board text-gray-600">
                    @foreach (['Atandı', 'basladi', 'Devam Ediyor', 'Test Ediliyor', 'Tamamlandı'] as $index => $status)
                        ...
                        <div class="p-4 rounded-lg shadow-lg min-h-[200px] md:min-h-[250px] lg:min-h-[300px] bg-white"
                            ...
                            @foreach ($userTasks->where('status', $status) as $task)
                                <div class="task-card p-4 mb-4 rounded-lg shadow-md relative bg-white" draggable="true"
                                    ...
                                    <div class="flex justify-between items-center mb-2">
                                        <h3 class="font-semibold text-md sm:text-lg truncate max-w-xs">{{ $task->title }}
                                        </h3>
                                    ...
                                    <div class="flex justify-between items-center">
                                        <div class="flex items-start">
                                            <span
                                                class="italic font-light text-gray-400">{{ $task->assignedUser->name ?? 'Kullanıcı yok' }}</span>
                                            <img class="h-6 w-6 rounded-full ml-3"
                                                src="{{ asset('images/' . ($task->assignedUser->profilePic ?? 'default.png')) }}"
                                                alt="{{ $task->assignedUser->name ?? 'Kullanıcı' }}" />
                                        </div>
