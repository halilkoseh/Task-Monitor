...
<div class="content-container mx-auto">
    ...
    <div class="flex flex-wrap justify-between items-center mb-8 p-2">
        <div class="search-container relative ml-8 w-full sm:w-auto sm:flex-grow">
            <input type="text" id="user-search" placeholder="Kullanıcı, Proje veya Task Arayın..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
            ...
            <div class="relative">
                <div class="project-box bg-white rounded-md shadow-md p-4">
                    ...
                    @foreach ($projects->take(2) as $index => $project) @php $iconBackgroundColors = ['bg-sky-100',
                    'bg-green-100', 'bg-orange-100']; $iconColors = ['text-sky-500', 'text-green-500',
                    'text-orange-500']; $iconData = [ ['icon'
                    => 'fa-code'], ['icon' => 'fa-project-diagram'], ['icon' => 'fa-tasks'], ['icon' => 'fa-rocket'],
                    ['icon' => 'fa-chart-network'], ['icon' => 'fa-clipboard-list'], ['icon' => 'fa-chart-pie'], ['icon'
                    => 'fa-code-branch'],
                    ['icon' => 'fa-database'], ['icon' => 'fa-desktop'], ['icon' => 'fa-file-code'], ['icon' =>
                    'fa-folder'], ['icon' => 'fa-laptop-code'], ['icon' => 'fa-microchip'], ['icon' => 'fa-mobile-alt'],
                    ['icon' =>
                    'fa-network-wired'], ['icon' => 'fa-server'], ['icon' => 'fa-shield-alt'], ['icon' =>
                    'fa-tablet-alt'], ]; $bgColor = $index == 1 ? 'bg-light-green-100' : $iconBackgroundColors[$index %
                    count($iconBackgroundColors)];
                    $iconColor = $index == 1 ? 'text-light-green-500' : $iconColors[$index % count($iconColors)]; $icon
                    = $iconData[$index % count($iconData)]; @endphp
                    ...
                    <div class="project-card bg-white rounded-md shadow-sm mb-4 p-4 flex items-center space-x-4">
                        ...
                        <div class="text-container flex-1">
                            ...
                            <span class="badge mt-2 inline-block {{ $bgColor }} text-gray-600">{{ $project->type
                                }}</span>
                        </div>
                    </div>
                    @endforeach
        ...
        <div class="bg-transparent py-3 px-3">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.users.assaign') }}">
                    <button id="add-task-btn"
                        class="p-1 px-3 bg-sky-500 text-white text-sm rounded-full hover:bg-blue-500">
                        <i class="fa-solid fa-circle-plus"></i> Görev Ata
                    </button>
                </a>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 task-board text-gray-600">
                ...
                <div class="p-4 rounded-lg shadow-lg min-h-[200px] md:min-h-[250px] lg:min-h-[300px] bg-white"
                    ...
                    data-status="{{ $status }}" ondragover="event.preventDefault()" ondrop="handleDrop(event)">
                    <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="task-card p-4 mb-4 rounded-lg shadow-md relative bg-white" draggable="true"
                        data-task-id="{{ $task->id }}" ondragstart="handleDragStart(event)"
                        ondragend="handleDragEnd(event)">
                        <div class="flex justify-end items-center">
                            <span
                                class="badge {{ ['bg-orange-100', 'bg-[#dcedc8]', 'bg-sky-100'][$loop->index % 3] }} text-gray-600 py-1 px-2 rounded-lg mb-2 inline-block ml-auto">
                        ...
                        <div class="flex justify-between items-center">
                            <div class="flex items-start">
                                ...
                                <img class="h-6 w-6 rounded-full ml-3" src="{{ asset('images/' . $user->profilePic) }}"
                                    alt="{{ $user->name }}" />
                            </div>
                            <div class="ml-4 space-x-2">
                                ...
                                <a href="{{ route('attachments.download', ['filename' => basename($task->attachments)]) }}"
                                    class="text-gray-600 hover:text-blue-500"><i class="fa-solid fa-paperclip"></i></a>
                                @endif
    
                                @if ($task->document)
    
                                <a href="{{ route('document.download', ['filename' => basename($task->document)]) }}"
                                    class="text-gray-600 hover:text-blue-500"><i class="fa-solid fa-file-pdf"></i></a>
                                @endif
                            </div>
                        </div>
