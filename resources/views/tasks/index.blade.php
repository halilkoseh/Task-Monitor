@extends('layout.app') @section('content')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div class="bg-white p-10 rounded-lg shadow-lg relative">
                    <a href="{{ route('admin.users.show') }}"
                        class="see-all-link absolute top-2 right-2 text-sky-500 hover:text-blue-600">Tümünü Gör</a>
                    ...
                    <div class="flex -space-x-2 overflow-hidden">
                        ...
                        @foreach ($users->take(10) as $user)
                            ...
                            @endforeach @if ($users->count() > 10)
                                <div
                                    class="inline-block h-10 w-10 rounded-full ring-2 ring-white bg-gray-300 flex items-center justify-center text-xs font-medium text-gray-700">
                                    +{{ $users->count() - 10 }}
                                </div>
                            @endif
                    </div>
    ...
    <div class="relative">
        <div class="project-box bg-white rounded-md">
            <h2 class="text-xl text-gray-600 font-semibold mt-2 -4">Projeler</h2>
            ...
            <div class="bg-transparent py-3 px-3">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-4 task-board text-gray-600">
                    @foreach (['Atandı', 'Başladı', 'Devam Ediyor', 'Test Ediliyor', 'Tamamlandı'] as $index => $status)
                        <div class="p-4 rounded-lg shadow-lg min-h-[300px]"
                            style="background-color: {{ ['#e0f2fe', '#f3e8ff', '#ffedd5', '#e0f2fe', '#f3e8ff'][$index % 5] }}"
                            ...
                            @foreach ($userTasks as $user)
                                @foreach ($user->tasks as $task)
                                    @if ($task->status == $status)
                                        <div class="task-card p-4 mb-4 rounded-lg shadow-md" draggable="true"
                                            data-task-id="{{ $task->id }}" ondragstart="handleDragStart(event)"
                                            ondragend="handleDragEnd(event)">
                                            <h3 class="font-semibold mb-2">{{ $task->title }}</h3>
                                            <span
                                                class="badge {{ ['bg-orange-100', 'bg-purple-100', 'bg-sky-100'][$index % 3] }} text-gray-600">{{ $task->projectName?->name }}</span>
                                            <p class="text-sm text-gray-700 mb-1">Görev İçeriği: {{ $task->description }}
                                            </p>
                                           ...
                                           <div class="flex justify-end"> 
                                            <img class="h-6 w-6 rounded-full flex justify-end items-end right-0 mr-0" src="{{ asset('images/' . $user->profilePic) }}" alt="{{ $user->name }}">
