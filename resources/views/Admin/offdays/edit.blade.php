...
<div class="content-container bg-gray-100">
    <div class="max-w-4xl w-full bg-white rounded-lg shadow-lg p-6">
        ...
        <form action="{{ route('admin.offdays.update', $offday->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="reason" class="block text-sm font-medium text-gray-700">Mazeret</label>
                <input type="text" name="reason" id="reason" value="{{ $offday->reason }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            </div>
            ...
            <div>
                ...
                @if ($offday->document)
                <p class="mt-2 text-sm text-gray-500">Mevcut Belge: <a href="{{ asset('storage/' . $offday->document) }}" target="_blank" class="text-sky-500 hover:underline">Belgeyi Görüntüle</a></p>
                @endif
            </div>
