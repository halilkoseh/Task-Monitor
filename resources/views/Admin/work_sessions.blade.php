    ...
    <div class="container mx-auto p-4">
        <div class="flex justify-between items-center mb-6">
            ...
            <form action="{{ route('admin.filterWorkSessions') }}" method="GET" class="flex items-center space-x-2">
                <select name="user_id" class="border border-gray-300 rounded-lg py-2 px-4 focus:ring-2 focus:ring-sky-500 focus:outline-none">
                    <option value="">Kullanıcı Seçin</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}">{{ $user->name }}</option>
                    @endforeach
                </select>
                <button type="submit" class="flex items-center px-4 py-2 bg-sky-500 text-white rounded-lg hover:bg-blue-500 transition duration-200">
        ...
        @foreach($workSessions as $session)
            <tr class="table-row">
                <td class="py-4 px-6 border-b border-gray-300 flex items-center">
                    ...
                    <span class="text-md font-medium">{{ $session->user->name }}</span>
                </td>
                ...
                <td class="py-4 px-6 border-b border-gray-300">
                    @php
                        $badgeColor = $session->status === 'completed' ? 'bg-green-500 text-white' : ($session->status === 'working' ? 'bg-purple-100 text-gray-600' : 'bg-orange-100 text-gray-600');
                    @endphp
                    <span class="inline-block px-3 py-1 text-sm font-semibold leading-tight rounded-full {{ $badgeColor }}">
                        {{ ucfirst($session->translated_status) }}
                    </span>
                </td>
                <td class="py-4 px-6 border-b border-gray-300">
                    @foreach($session->breaks as $break)
                        <span class="badge badge-start">{{ \Carbon\Carbon::parse($break->start_time)->format('d/m/Y H:i') }}</span>
                    @endforeach
                </td>
                <td class="py-4 px-6 border-b border-gray-300">
                    @foreach($session->breaks as $break)
                        <span class="badge badge-end">{{ \Carbon\Carbon::parse($break->end_time)->format('d/m/Y H:i') }}</span>
                    @endforeach
                </td>
                <td class="py-6 px-6 border-b border-gray-300 relative">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-content">
                            <a href="{{ route('admin.editWorkSession', $session->id) }}" class="block px-4 py-2 text-blue-600 hover:bg-blue-100">Düzenle</a>
                            <form action="{{ route('admin.destroyWorkSession', $session->id) }}" method="POST" onsubmit="return confirm('Bu oturumu silmek istediğinize emin misiniz?');" class="block">
                                @csrf
                                @method('DELETE')
