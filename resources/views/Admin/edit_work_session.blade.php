        ...
        <form action="{{ route('admin.updateWorkSession', $workSession->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" name="start_time"
                    value="{{ optional($workSession->start_time)->format('Y-m-d\TH:i') }}"
            ...
            <div class="mb-6">
                ...
                @foreach ($workSession->breaks as $break)
                    <div class="mb-4">
                        <input type="datetime-local" name="breaks[{{ $break->id }}][start_time]"
                            value="{{ optional($break->start_time)->format('Y-m-d\TH:i') }}"
                        ...
                        <input type="datetime-local" name="breaks[{{ $break->id }}][end_time]"
                            value="{{ optional($break->end_time)->format('Y-m-d\TH:i') }}"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 mb-2">
                    </div>
                @endforeach
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" name="end_time"
                    value="{{ optional($workSession->end_time)->format('Y-m-d\TH:i') }}"
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div class="text-center">
                <button type="submit"
                    class="px-6 py-3 text-white bg-blue-500 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">Güncelle</button>
            </div>
        </form>
    </div>
@endsection
